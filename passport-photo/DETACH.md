...
Subdomain (same Vercel project): point a CNAME such as `photospec.averonsoft.com`
...
- `NEXT_PUBLIC_DOMAIN` / `NEXT_PUBLIC_BASE_URL` (e.g. `https://photospec.averonsoft.com`)
...
- **Same project as Averonsoft:** set `PASSPORT_PHOTO_HOST=photospec.averonsoft.com` (or your CNAME) on the parent app.
...
- **Root / CNAME** (e.g. `https://photospec.averonsoft.com`): leave `NEXT_PUBLIC_BASE_PATH` empty.
