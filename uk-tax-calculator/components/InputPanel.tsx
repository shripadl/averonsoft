...
function Field({
  ...
  return (
    <label className="flex flex-col gap-1.5">
      <span className="text-[13px] font-medium tracking-wide text-[var(--ink-soft)]">
        {label}
      </span>
      {children}
      {hint ? (
        <span className="text-[11px] leading-snug text-[var(--muted)]">{hint}</span>
      ) : null}
    </label>
  );
}
...
function Section({
  ...
  return (
    <div className="payframe-section">
      <div className="mb-4 flex items-baseline gap-2">
        <span className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--teal)]">
          {eyebrow}
        </span>
...
const controlClass = "payframe-control";
...
export function InputPanel({ value, onChange, title = "Your pay" }: Props) {
  ...
  return (
    <section className="payframe-panel">
      <div className="payframe-panel__accent" aria-hidden />
      <div className="payframe-panel__body">
        <h2
          className="mb-6 text-xl tracking-tight text-[var(--ink)]"
        ...
        <div className="space-y-5">
          <Section eyebrow="01" title="Earnings">
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Gross pay">
                <div className="relative">
                  <span className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-sm font-medium text-[var(--teal-deep)]">
                    £
                  </span>
                  <input
              ...
              {showHours ? (
                <Field label="Hours each week" hint="Used to annualise hourly pay.">
                  <input
                    ...
                    onChange={(e) =>
                      set("hoursPerWeek", Number(e.target.value) || 0)
                    }
                  />
                </Field>
              ) : null}
              ...
              {showDays ? (
                <Field
                  ...
                  hint="Used for daily and hourly conversion."
                >
                  <input
                    className={controlClass}
                    type="number"
                    ...
                    onChange={(e) =>
                      set("daysPerWeek", Number(e.target.value) || 5)
                    }
                  />
                </Field>
              ) : null}
          ...
          <Section eyebrow="02" title="Tax & NI">
            <div className="grid gap-4 sm:grid-cols-2">
              <Field
                label="Tax code"
              ...
              <Field label="Age band" hint="Kept for future NI letter rules.">
                ...
                >
                  <option value="under_state_pension">
                    Under State Pension age
                  </option>
                  <option value="state_pension_age">
              ...
              <Field label="NI category letter">
                <select
                  ...
                  onChange={(e) =>
                    set(
                      "niCategory",
                      e.target.value as CalculatorInput["niCategory"],
                    )
                  }
                >
          ...
          <Section eyebrow="03" title="Pension & extras">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    set("otherIncome", Number(e.target.value) || 0)
                  }
                />
              </Field>
          ...
          <Section eyebrow="04" title="Allowances">
...
function Toggle({
  ...
  return (
    <label
      className={`payframe-toggle ${checked ? "payframe-toggle--on" : ""}`}
    >
      <input
        type="checkbox"
        className="sr-only"
      ...
      />
      <span className="payframe-toggle__mark" aria-hidden />
      <span className="text-sm leading-snug text-[var(--text)]">{label}</span>
    </label>
  );
}
